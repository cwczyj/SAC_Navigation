...
def get_configs():
    ...
    house_file = os.path.join(cur_path, '../path_data/human_house_split.json')
    with open(house_file, 'r') as f:
        house_data = json.load(f)
    train_houses = house_data['train']
    val_houses = house_data['val']
    test_houses = house_data['test']
    fix_test_house_ids = []
    try:
    ...
    configs = {
        ...
        'render_width': 224,
        'output_height': 224,  # height and width for rgb image to the agent/network
        'output_width': 224,
        'par_path': cur_path,
        'path': config_path,
        'move_sensitivity': 0.3,  # forward movement distance in one step
        'rot_sensitivity': 20,  # rotation angle in one step
